...
int eval(int p, int q, bool *success){
    if(p>q){
        ...
        if(tokens[p].type == NUM){
            ...
            if(strcasecmp(reg, "eax") == 0){
                return cpu.eax;
            }
            if(strcasecmp(reg, "ecx") == 0){
                return cpu.ecx;
            }
            if(strcasecmp(reg, "edx") == 0){
                return cpu.edx;
            }
            if(strcasecmp(reg, "ebx") == 0){
                return cpu.ebx;
            }
            if(strcasecmp(reg, "esp") == 0){
                return cpu.esp;
            }
            if(strcasecmp(reg, "ebp") == 0){
                return cpu.ebp;
            }
            if(strcasecmp(reg, "esi") == 0){
                return cpu.esi;
            }
			if(strcasecmp(reg,"edi") == 0){
				return cpu.edi;
			}
            for(i = 0; i < 8; ++i)
