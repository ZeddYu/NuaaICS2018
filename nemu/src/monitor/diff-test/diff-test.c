...
void difftest_step(uint32_t eip) {
  ...
  for(index = 0; index < 8; index++){
	if(r.array[index] != reg_l(index)) {
		diff = true;
		printf("At $eip : 0x%08x, REG $%s Qemu 0x%08x ; Nemu 0x%08x\n", cpu.eip, reg_name(index, 4), r.array[index], reg_l(index));
		break;
	}
  }
  if(r.eip != cpu.eip){
	diff = true;
	printf("At $eip : 0x%08x, REG $eip Qemu 0x%08x; Nemu 0x%08x\n", cpu.eip, r.eip, cpu.eip);
  }
  if (diff) {
    nemu_state = NEMU_END;
  }
